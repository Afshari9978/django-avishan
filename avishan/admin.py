...
for model in AvishanModel.get_non_abstract_models():
    model_admin_dict = {
        'list_filter': [field.name for field in model.django_admin_list_filter],
        'list_max_show_all': model.django_admin_list_max_show_all,
        'list_per_page': model.django_admin_list_per_page,
        'raw_id_fields': [field.name for field in model.django_admin_raw_id_fields],
        'readonly_fields': [field.name for field in model.django_admin_readonly_fields],
        'search_fields': [field.name for field in model.django_admin_search_fields],
    }
    ...
    if len(model.django_admin_list_display) > 0:
        model_admin_dict['list_display'] = [field.name for field in model.django_admin_list_display]
